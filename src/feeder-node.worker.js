...
self.onmessage = async function(e) {
	if (e.data.command == 'feed') {
		...
		await init(converterType, nChannels, inputSampleRate, outputSampleRate, pathToWasm);
...
function feed(data) {
	...
	if (backendPort == undefined) {
		...
		backendPort.postMessage({command: 'feed', data: resampled}, [resampled.buffer]);
	}
}
 ...
 * 
 */
async function init(converterType, nChannels, inputSampleRate, outputSampleRate, wasmPath) {
